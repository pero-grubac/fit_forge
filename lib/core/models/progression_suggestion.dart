...
class ProgressionSuggestion {
  ...
  final String confidence; // High | Hold | Reduce | NoData
  final String reason;
  ...
  bool get hasData => confidence != 'NoData';

  bool get shouldIncrease => confidence == 'High';

  static const noData = ProgressionSuggestion(
    suggestedWeights: [],
    suggestedReps: [],
    confidence: 'NoData',
    reason: 'Nedovoljno podataka — odradite jos treninga',
  );
}
...
class ProgressionCalculator {
  ...
  ProgressionSuggestion calculate(List<WorkoutLogModel> logs) {
    ...
    final avgRate =
        recent.map(_completionRate).reduce((a, b) => a + b) / _historyNeeded;
